...
- [新功能] 新增银行股价值框架策略 `tianjie_bank_stock_investment`，提供基于股息率、PB 历史分位、现金流-净资产匹配度与净息差趋势的专用分析与仓位建议。
...
- 🧭 **新增“天阶价值投资策略”默认激活与自动路由接入** — 内置策略 `tianjie_value_investment` 现已加入默认 `AGENT_SKILLS` 激活集合，并接入 `StrategyRouter` 的 `trending_down/sideways` 场景与默认 fallback 集合，`AGENT_STRATEGY_ROUTING=auto` 下可按市场状态自动命中该策略。
